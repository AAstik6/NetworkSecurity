...
class TrainingPipeline:
  ...
  def start_data_ingestion(self):
    try:
      ...
      logging.info(f"data ingestion artifact is completed {data_ingestion_artifact}")
  ...
  def start_data_validation(self, data_ingestion_artifact):
    try:
      ...
      logging.info("data validation artifact completed.")
      return data_validation_artifact
    except Exception as e:
      raise NetworkSecurityException(e,sys)
  
  def start_data_transforamtion(self, data_validation_artifact):
    try:
  ...
  def sync_artifact_dir_to_s3(self):
    try:
      aws_bucket_url = f"s3://{TRAINING_BUCKET_NAME}/artifact/{self.training_pipeline_config.artifact_dir}"
