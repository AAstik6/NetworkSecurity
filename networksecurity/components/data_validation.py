...
from scipy.stats import ks_2samp ## checks if there is a data drift or not. 
import pandas as pd
import os,sys
...
class DataValidation:
   ...
   def validate_number_of_columns(self, dataframe:pd.DataFrame)-> bool:
      try:
         ...
         logging.info(f"the number of cols the dataframe should have--> {number_of_columns}")
   ...
   def check_numeric_cols(self, dataframe):
        try:
            return all(dtype == "int64" for dtype in dataframe.dtypes)
        except Exception as e:
            raise NetworkSecurityException(e,sys)
